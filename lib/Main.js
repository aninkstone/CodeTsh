...
var NewDocument = require(set.runtime.load + "object/Document.js");

//var defaultDoc = NewDocument("./CodeTor.txt");
var defaultDoc = NewDocument("lib/Windows.js");
var consoleDoc = NewDocument("tmp/copen.txt");
...
require(set.runtime.load + "BenchMark.js");
...
function InteractText (text) {
    let inet = windows.namedCtrl("Inet");
    inet.handle.document.deleteChars(0, inet.handle.document.length);
    inet.handle.document.insertChars(text);
...
function ExecuteCommand (widget, cmd, shift, alt, ctrl){
    try {
        ...
        set.vim.script = set.vim.script ? set.vim.script : null;
        set.vim.number = set.vim.number ? set.vim.number : 0;
        let script = null;

        if (set.vim.script != null) {
            script = set.vim.script;
        }
        else {
        ...
        set.vim.cmd += cmd;
        InteractText("[keymap]:" + set.vim.cmd);
